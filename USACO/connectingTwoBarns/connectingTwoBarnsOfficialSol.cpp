...
ll cost(int a, int b) {
	...
	for (int u : comps[a]) {
		 ...
		 * component).
		 */
		auto i = upper_bound(comps[b].begin(), comps[b].end(), u);
		if (i != comps[b].end()) { dist = min(dist, abs((*i) - u)); }
		if (i != comps[b].begin()) { i--; dist = min(dist, abs((*i) - u)); }
	}
	/*
	 * Returns the minimum cost of building a path
	 * between the two components (aka dist^2)
	 */
	return 1LL *dist * dist;
}
