...
int main(){
    ...
    for (ll i = 0; i < m; i++){
        ll sj, cj;
        cin >> sj >> cj;
        for (ll x = 0; x < endpts.size(); x++){
            //binary search
            int l = 0;
            int r = endpts[x].size() - 1;
            while (l <= r && l >= 0 && r < endpts[x].size()){
                int mid = l + (r - l)/2;
                auto it = waypoints[endpts[x][mid]].s.upper_bound(sj);
                ll amt = distance(it, waypoints[endpts[x][mid]].s.end());
                if (amt <= cj){
                    ans[i] = max(ans[i], waypoints[endpts[x][mid]].f);
                    l = mid + 1;
                } else{
                    r = mid - 1;
                }
